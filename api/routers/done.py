...
async def mark_as_done(task_id: int, db: Session = Depends(get_db)):
...
async def unmark_as_done(task_id: int, db: Session = Depends(get_db)):
    done = done_crud.get_done(db, task_id=task_id)
    if done is None:
        raise HTTPException(status_code=400, detail="Done not Found")

    return done_crud.delete_done(db, task_id=task_id)
